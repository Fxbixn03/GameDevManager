...
- **Was nicht mehr hineinpasst, sammelt ein Aufklapp-Knopf am rechten Ende.** Wie viele Knöpfe hineinpassen, weiß nur der Browser: [gdm-topbar.js](src/GameDevManager.Web/wwwroot/js/gdm-topbar.js) misst mit einem `ResizeObserver` und meldet über `SetCapacity` **allein diese Anzahl**; welche Module das betrifft, entscheidet `MainLayout.BuildStrip`. Dieselbe Trennung wie bei den Karten — Geometrie im Browser, Fachlichkeit in C# —, und sie hat den praktischen Vorteil, dass ein abgeschaltetes Modul oder ein anderer Benutzer keine neue Messung braucht.
...
- **Die Leiste hat `flex: 1 1 0` und keinen `MudSpacer` mehr daneben.** Ihre Breite darf nicht an der Anzahl der Knöpfe hängen, sonst liefe die Messung im Kreis: weniger Knöpfe → schmalere Leiste → weniger Platz → noch weniger Knöpfe. Aus demselben Grund misst `gdm-topbar.js` ein eigenes `div.gdm-module-item` je Modul und nicht den MudBlazor-Knopf darin — dessen Aufbau (Tooltip-Hülle) gehört uns nicht.
