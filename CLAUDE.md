...
`ContentLanguage` (Sprachen je Projekt) und `ContentTranslation` (die Texte), bedient von `LocalizationService`, Oberfläche im Werkzeug-Modul `ModuleKeys.Localization`. Migration in allen vier Providern: `ContentLocalization`, `FormatVersion` des Exports auf **7**. Sieben Dinge:
...
- **Der `Slot` sagt, welcher Text gemeint ist**: `"name"`, `"description"` oder die GUID einer `FieldDefinition` — eine Textspalte, kein Verweis, dieselbe Überlegung wie beim `Slot` der Bedingungssätze. Neue Textfelder kommen damit ohne Änderung an dieser Stelle mit.
